...
import { Map, Plus, MapPin, CheckCircle, XCircle, Trash2, MoreVertical, Pencil, ToggleRight, Building2 } from 'lucide-react';
...
const MunicipiosMap = lazy(() => import('./Partials/MunicipiosMap'));
...
interface MunicipiosPageProps {
    auth: Auth;
    municipios: Paginated<Municipio>;
...
export default function MunicipiosIndexPage({
    ...
    const breadcrumbs = [
        ...
        { title: __('Municipalities'), href: '/admin/municipios' },
    ];
    ...
    const { data, setData, post, put, processing, errors, reset } = useForm({
        ...
        codigo: '',
        capital: '',
        latitud: 10.2354 as number | null,
        longitud: -67.5911 as number | null,
        activo: true,
    });
    ...
    }, [searchTerm, statusFilter, estadoFilter, paisFilter, perPageFilter]);
    ...
    const handleEditClick = (municipio: Municipio) => {
        ...
        setData({
            ...
            latitud: municipio.latitud !== undefined && municipio.latitud !== null ? Number(municipio.latitud) : 10.2354,
            longitud: municipio.longitud !== undefined && municipio.longitud !== null ? Number(municipio.longitud) : -67.5911,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingMunicipio) {
            put(`/admin/municipios/${editingMunicipio.id}`, {
                onSuccess: () => {
    ...
    const handleToggleStatus = (municipio: Municipio) => {
        router.post(`/admin/municipios/${municipio.id}/toggle-status`, {}, {
            preserveScroll: true,
        });
    };

    const handleConfirmBulkDelete = () => {
        router.post('/admin/municipios/bulk-destroy', { ids: selectedIds }, {
            onSuccess: () => {
                setSelectedIds([]);
    ...
    const filteredEstadosForFilter = paisFilter
        ? estados.filter(e => String(e.pais_id) === String(paisFilter))
        : estados;

    const columns: ColumnDef<Municipio>[] = [
        {
            accessorKey: 'nombre',
            header: __('Municipality'),
            cell: (row) => (
                <div className="flex flex-col">
                    <span className="font-semibold text-foreground">{row.nombre}</span>
                    {row.estado && (
                        <span className="text-xs text-muted-foreground">
                            {row.estado.nombre} {row.estado.pais ? `(${row.estado.pais.codigo_iso2})` : ''}
                        </span>
                    )}
                </div>
            ),
            ...
            cell: (row) => (
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        ...
                        {hasPermission('municipios.delete') && (
                            <DropdownMenuItem
                                onClick={() => {
    ...
    return (
        <>
            <Head title={__('Municipalities of Venezuela')} />
            ...
            <div className="space-y-6">
                ...
                <ModuleHeader
                    ...
                    title={__('Municipalities of Venezuela')}
                    description={__('Management of municipalities, capitals and geographic coordinates')}
                    colorClassName="bg-emerald-600"
                >
                    {hasPermission('municipios.create') && (
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-2 border-b">
                        ...
                        {selectedIds.length > 0 && hasPermission('municipios.delete') && (
                            <Button
                                variant="destructive"
                                size="sm"
                            ...
                            <div className="flex flex-wrap items-end gap-4 w-full">
                                ...
                                <FilterField label={__('State')}>
                                    <Select2
                                        ...
                                        value={estadoFilter}
                                        onChange={(val) => setEstadoFilter(String(val))}
                    ...
                    <TabsContent value="mapa" className="mt-4">
                        ...
                        >
                            <MunicipiosMap municipios={municipios.data} />
                        </Suspense>
                    </TabsContent>
                </Tabs>
            </div>
                ...
                <DialogContent className="sm:max-w-4xl w-[95vw] max-h-[90vh] overflow-y-auto p-6 sm:p-8">
                    <DialogHeader>
                        <DialogTitle className="text-xl">
                            {editingMunicipio ? __('Edit Municipality') : __('New Municipality')}
                        </DialogTitle>
                        <DialogDescription>
                            {__('Enter the geographic and administrative details of the municipality.')}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5 py-2">
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div className="space-y-2">
                                <Label htmlFor="estado_id">{__('State')} *</Label>
                                <Select2
                                    id="estado_id"
                                    options={estados.map((e) => ({
                                        value: e.id,
                                        label: e.nombre,
                                        sublabel: e.pais ? `${e.pais.nombre} (${e.pais.codigo_iso2})` : undefined,
                                    }))}
                                    value={data.estado_id}
                                    onChange={(val) => {
                                        const selectedEstado = estados.find(est => String(est.id) === String(val));
                                        setData(prev => ({
                                            ...prev,
                                            estado_id: Number(val),
                                        }));
                                    }}
                                    placeholder={__('Select a state')}
                                    searchPlaceholder={__('Search state...')}
                                    emptyText={__('No state found')}
                                    className="w-full"
                                />
                                {errors.estado_id && <p className="text-xs text-destructive">{errors.estado_id}</p>}
                            </div>

                            <div className="space-y-2">
                                <Label htmlFor="nombre">{__('Municipality Name')} *</Label>
                                <Input
                                    id="nombre"
                                    value={data.nombre}
                                    onChange={(e) => setData('nombre', e.target.value)}
                                    placeholder="Ej: Girardot, Maracaibo, Chacao..."
                                    required
                                />
                            ...
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    placeholder="Ej: VE-D-GIR, VE-V-MAR..."
                                />
                                {errors.codigo && <p className="text-xs text-destructive">{errors.codigo}</p>}
                            </div>

                            <div className="space-y-2">
                                <Label htmlFor="capital">{__('Capital / Municipal Seat')}</Label>
                                <Input
                                    id="capital"
                                    value={data.capital}
                                    onChange={(e) => setData('capital', e.target.value)}
                                    placeholder="Ej: Maracay, Maracaibo, El Limón..."
                                />
                        ...
                        <div className="border-t pt-4 space-y-3">
                            ...
                            <div className="pt-2">
                                <MapboxMap
                                    lat={data.latitud ?? 10.2354}
                                    lng={data.longitud ?? -67.5911}
                                    zoom={8}
                                    interactive={true}
                        ...
                        <div className="flex items-center justify-between border-t pt-4">
                            <Label htmlFor="activo" className="flex flex-col gap-1 cursor-pointer">
                                <span>{__('Active Municipality')}</span>
                                <span className="text-xs font-normal text-muted-foreground">
                                    {__('Enable or disable this municipality in application dropdowns.')}
                                </span>
                            </Label>
                            <Switch
                                id="activo"
                        ...
                        <DialogFooter className="pt-4">
                            ...
                            <Button type="submit" disabled={processing}>
                                {editingMunicipio ? __('Save Changes') : __('Create Municipality')}
                            </Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
            ...
            <DeleteConfirmationDialog
                ...
                description={
                    selectedIds.length > 1
                        ? __('This action will permanently delete the :count selected municipalities.', { count: String(selectedIds.length) })
                        : __('This action will permanently delete the selected municipality.')
                }
            />
        </>
    );
